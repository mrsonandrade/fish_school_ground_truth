...
class HeroExample(object):
    ...
    def run(self):
        ...
        current_goal = 0
        waypoints = np.array([[-1.0,  1.0, 0.0], 
                              [ 1.0,  1.0, 0.0], 
                              [ 1.0, -1.0, 0.0], 
                              [-1.0, -1.0, 0.0]])
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                goal = waypoints[current_goal]
                distance = np.linalg.norm(goal - robot_positions[r_i_index])
                if distance < 0.05:
                    current_goal = (1+current_goal) % len(waypoints)
                ...
                print("Goal #{} - error: {} - u: [{},{}]".format(
                    current_goal, distance, vel.linear.x, vel.angular.z))
